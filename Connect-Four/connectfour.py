...
class ConnectFourBoard(object):
    ...
    board_height = 6
    #If this game is k length or longest streak #####Added
    longest_streak = False
    #default value of k #####Added
    k = 4
    ...
    def __init__(self, k = 4, board_array = None, board_already_won = None, modified_column = None, current_player = 1, previous_move = 
    -1, longest_streak = False):
    ...
    def get_current_winning_k(self):
        """ Stores the k-connect game input """
        return self.k
    ...
    def get_longest_streak(self):
        """
        Used to store user input of game
        """
    ...
    def do_move(self, column):
        ...
        """Inclreasing number of nodes explored """
        global nodes_count
        nodes_count += 1
            
        ####################################################
        ###Modified to send connect-k and longest streak bool in case of Longest Streak.
        ####################################################
        return ConnectFourBoard(self.get_current_winning_k(), new_board, modified_column=column, current_player = 
        self.get_other_player_id(),longest_streak = self.get_longest_streak())
    ...
    def _is_win_from_cell(self, row, col):
        ...
        return ( self._max_length_from_cell(row, col) >= self.get_current_winning_k())
    ...
    def is_win(self, longest_streak = False):
        """
        Return the id# of the player who has won this game.
        Return 0 if it has not yet been won.
        """
        #########################################
        ###Modified to detect the win in case of Longest Streak.
        #########################################
        if(self.get_longest_streak() == True):
            if(longest_streak == True):
                l1 = self.longest_chain(self.get_current_player_id())
                l2 = self.longest_chain(self.get_other_player_id())
                if(l1 == l2):
                    return 0
                else:
                    self._is_win = True
                    return self.get_current_player_id() if l1 > l2 else self.get_other_player_id()
        else:
            for i in xrange(self.board_height):
                for j in xrange(self.board_width):
    ...
    def is_tie(self, longest_streak = False):
        """ Return true iff the game has reached a stalemate """
        #########################################
        ###Modified to detect the ties in case of Longest Streak.
        #########################################
        if(self.get_longest_streak() == True):
            if(longest_streak == True):
                l1 = self.longest_chain(self.get_current_player_id())
    ...
    def clone(self):
        ...
        ###Modified to send connect-k and longest streak bool in case of Longest Streak.
        ####################################################
        return ConnectFourBoard(self.get_current_winning_k(), self._board_array, current_player = 
        self.get_current_player_id(),longest_streak = self.get_longest_streak())
...
class ConnectFourRunner(object):
    ...
    * The game ends when one of the two players has four consecutive tokens in a row
    ...
    def __init__(self, player1_callback, player2_callback,longest_streak = 
                        False, board = ConnectFourBoard(), time_limit = 10):
    ...
    def print_node_and_time(self,tic):
        """ Calucluating run time and printing """
        toc = time.clock()
        timeItr = toc - tic
        print "Execution Time: " + str(timeItr)
        print "Nodes Expanded: " + str(nodes_count)
    ...
    def run_game(self, verbose=True):
        ...
        win_for_player = []
        ply = 0
        if (self.longest_streak == True):
            check = self.longest_streak and ply < max_ply
        else:
            check = not win_for_player and not self._board.is_tie()
        while check:
            for callback, id, symbol in ( player1, player2 ):
                if(self.longest_streak == True and ply == max_ply-1):
                    ply= ply+1
                    break;
                if verbose:
                ...
                while not has_moved:
                    try:
                        ...
                        if sys.stdout.encoding and 'UTF' in sys.stdout.encoding:
                            ...
                            continue
                """game will not terminate in between if it is longest streak game"""
                if not self.longest_streak:
                    if self._board.is_game_over():
                        print "Game Over"
                        win_for_player = self._board.is_win()
                        break

            ###########to update the check of while ##########
            if (self.longest_streak == True):
                check = self.longest_streak and ply < max_ply
            else:
                check = not win_for_player and not self._board.is_tie()
        ...
        """Deciding the winner or tie based on game type here"""
        if (self.longest_streak == True):
            win_for_player = self._board.is_win(self.longest_streak)
            if win_for_player == 0 and self._board.is_tie(self.longest_streak):
                print "It's a tie!  No winner is declared."
                self.print_node_and_time(tic)
                return 0
            else:
...
def run_game(player1, player2, board = ConnectFourBoard(),longest_streak = False):
    """ Run a game of Connect Four, with the two specified players """
    game = ConnectFourRunner(player1, player2, board=board, longest_streak = longest_streak)

    """let's store longest streak game or not"""
    board.set_longest_streak(longest_streak)
