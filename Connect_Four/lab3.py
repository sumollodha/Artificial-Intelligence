...
# Email: <risagrawal@cs.stonybrook.edu>
# Name: <Sumol Lodha>
# Email: <slodha@cs.stonybrook.edu>
...
def alpha_beta_util(board, depth, eval_fn, get_next_moves_fn,
                        is_terminal_fn,max_index, alpha, beta):
    """using negmax varient of alpha beta algorithm"""
    if(depth <= 0):
        """using pow of depth instaed of passing an extra argument"""
        return (pow(-1, depth)* eval_fn(board))

    """strating with negative Infinity"""
    currentUtil = NEG_INFINITY
    for i,new_board in get_next_moves_fn(board):
        if(is_terminal_fn(depth, new_board) != True):
            utilVal = -alpha_beta_util(new_board, depth -1, eval_fn, get_next_moves_fn,
            is_terminal_fn,max_index, alpha, beta)
...
def alpha_beta_search(board, depth,
      ...
      max_index = []
      alpha_beta_util(board, depth, eval_fn, get_next_moves_fn,
                      is_terminal_fn, max_index,INFINITY, NEG_INFINITY)
      column = 0
...
############################
##Execution Time: 60.148703975
##Nodes Expanded: 28214
############################
run_game(basic_player, new_player)

############################
##Execution Time: 63.101704486
##Nodes Expanded: 28214
############################
#run_game(new_player, new_player)

#run_game(alphabeta_player, alphabeta_player)
#run_game(alphabeta_player, basic_player, ConnectFourBoard(5))
#run_game(alphabeta_player, alphabeta_player,longest_streak = True)
